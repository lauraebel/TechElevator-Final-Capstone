...
public class JdbcToolDAO implements ToolDAO {
	...
	public List<Tool> getAllAvailableTools() {
		...
		String sql = "SELECT "
							...
							+ "tools.brand_id "
				+ "FROM tools "
				+ "FULL JOIN loans ON tools.id=loans.tool_id "
				+ "FULL JOIN reservations ON tools.id=reservations.tool_id "
				+ "WHERE tools.id NOT IN (SELECT tool_id FROM loans WHERE returned_on IS NULL) "
				+ "AND tools.id NOT IN (SELECT tool_id FROM reservations)";
