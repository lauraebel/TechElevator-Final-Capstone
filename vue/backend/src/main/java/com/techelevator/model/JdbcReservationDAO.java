...
public class JdbcReservationDAO implements ReservationDAO {
	...
	public List<Reservation> searchByBorrowerLicenseNumber(String licenseNumber) {
		...
		String sql = "SELECT reservations.id, reservations.user_id, reservations.tool_id, "
				+ "reservations.loaned_on, reservations.due_on, " + "reservations.returned_on FROM reservations "
				+ "JOIN users ON reservations.user_id=users.id" + "WHERE users.license_number= (CAST (? AS varchar))";
	...
	public Reservation renewReservation(long reservationId) {
		...
		SqlRowSet reservationResults = jdbcTemplate.queryForRowSet(SQL_SELECT_RESERVATION + "WHERE id = ?",
				reservationId);
